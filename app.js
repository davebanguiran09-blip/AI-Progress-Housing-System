...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init() {
        // -------------------------------------------------------
        // DISCLAIMER MODAL — show every time Custom Build is opened
        // -------------------------------------------------------
        const disclaimerModal = document.getElementById('customBuildDisclaimerModal');
        const disclaimerProceedBtn = document.getElementById('disclaimerProceedBtn');

        if (disclaimerModal) {
            // Only show the warning once per session (not again if they navigate back and return)
            const alreadyAccepted = sessionStorage.getItem('customBuildAccepted');
            ...
            if (disclaimerProceedBtn) {
                disclaimerProceedBtn.addEventListener('click', () => {
                    sessionStorage.setItem('customBuildAccepted', '1'); // remember for this session
                    disclaimerModal.classList.remove('active');
                    document.body.style.overflow = ''; // restore scrolling
                });
            }

            // Also close if user clicks the backdrop area (counts as acceptance)
            disclaimerModal.addEventListener('click', (e) => {
                if (e.target === disclaimerModal) {
                    sessionStorage.setItem('customBuildAccepted', '1');
                    disclaimerModal.classList.remove('active');
